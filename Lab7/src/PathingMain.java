...
public class PathingMain extends PApplet
{
   ...
   private static void initialize_grid(GridValues[][] grid)
   {
      ...
      for(int col = 14; col < 19; col++ ){
         grid[col -9][col] = GridValues.OBSTACLE;
      }
      ...
      for(int row = 12; row <14; row++){
         grid[row][row-10] = GridValues.OBSTACLE;
      }
...
//      }
//      redraw();
      Point rP = new Point(pos.x + 1, pos.y);
      Point dP = new Point(pos.x, pos.y + 1);
      Point lP = new Point(pos.x - 1, pos.y);
      ...
      if (withinBounds(pos, grid) && grid[pos.y][pos.x] != GridValues.OBSTACLE && grid[pos.y][pos.x] != GridValues.SEARCHED) {
         ...
         if (moveOnce(rP, grid, path) || moveOnce(dP, grid, path) || moveOnce(lP, grid, path) || moveOnce(uP, grid, path)) {
            path.add(pos);
            return true;
         }
      }
